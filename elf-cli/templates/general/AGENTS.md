...
이 프로젝트는 **ELF(Eli's Lab Framework) general preset**(목표지향 비연구) 거버넌스를 따름. 본 파일은 AI 에이전트 진입용 **요약(digest)** — 규칙 정본은 `0_Meta/`. 요약과 정본이 다르면 **정본 우선**. (ELF 관리 파일 — 직접 수정 금지, `elf update`가 교체.)

## 규칙 정본 (필독)

| 파일 | 역할 |
|---|---|
| `0_Meta/EliRule.md` | 전역 규칙 — 폴더 구조, §3 AI 소통(언어·문체·금지) |
| `0_Meta/LogConvention.md` | **세션 로그·trial 작성 규칙(필수 준수)** — 포맷·Phase 절차 |
...
- ELF 관리 파일(`0_Meta/`의 EliRule·LogConvention·AI_PARA_Framework·LLMcliche, `templates/*`, `.claudeignore`, 본 파일) **직접 수정 금지** — `elf update`가 교체(편집 시 `.elf-new` 병기). 커스터마이즈·예외 선언은 `0_Meta/ProjectRule.md`.
