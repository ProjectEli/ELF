...
the `.claudeignore` rules. For the detailed principles, see `.elf/managed/AI_PARA_Framework.md`.
...
### Rules
- **Writing language**: write logs in the language set by `PROJECT_LANG` in `.elf/managed/EliRule.md`, but for **token economy** use nominal endings ('-음/함/임') and word-centered bullet points. English may be added for technical terms.
...
- **Registry key finding**: the same fold principle as Handoff — replace-update it with the currently valid final conclusion (not an accumulating narrative), and rewrite it as the final fold at session close (§5.1).
