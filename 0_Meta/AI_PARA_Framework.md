...
*   **운영 규칙**: 별도의 `1_Active` 폴더를 만들지 않고, **작업 중인 부모 폴더(예: `13_Planning/` 또는 `7_Log/`)의 최상단(Root)**을 바로 작업대로 사용함. 작업이 완료되어 과거의 기록이 되기 전까지는 이 공간에서 자유롭게 작성함.
...
*   *(사용자 프롬프트 예시)*: "`7_Log/9_Archive/S005_log.md` 파일을 열어서 당시의 파라미터 값 추이를 요약해 줘."
