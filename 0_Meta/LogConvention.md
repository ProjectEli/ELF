...
| 항목 | 규칙 |
|------|------|
| **진행 중 (Active Sandbox)** | 새로운 세션(S{NNN})을 시작할 때는 무조건 **`7_Log/`의 최상단(Root)**에 로그(`S{NNN}_log.md`)를 생성하여 작성합니다. |
...
| **금지 사항** | 아이디어, 논문 기획, 방향성 논의는 `7_Log/`에 기록하지 말고 `1_Concept/13_Planning/`의 기획 문서로 분리합니다. |
...
- **이미지 경로**: `![alt text](../5_Exp/54_Viz/S{NNN}/filename.png)` (7_Log 기준 상대경로)
...
| 종류 | 위치 | 비고 |
|------|------|------|
| 자동 생성 Figure | `5_Exp/54_Viz/` | 스크립트에서 자동 저장. 산출 후 `0_Meta/scripts/elf_gallery` 실행하여 `_gallery.md` 자동 갱신 권장. |
...
| From | To | 상대경로 (7_Log 기준) |
|------|----|----------------------|
| Logs → Planning | `1_Concept/13_Planning/P00x.md` | `../1_Concept/13_Planning/P00x.md` |
| Logs → Sim Data | `5_Exp/51_Sim/Data/S{NNN}/` | `../5_Exp/51_Sim/Data/S{NNN}/` |
| Logs → Viz | `5_Exp/54_Viz/S{NNN}/` | `../5_Exp/54_Viz/S{NNN}/` |
...
- [ ] `7_Log/S{NNN}_log.md` 생성 (위 포맷 준수)
- [ ] `5_Exp/51_Sim/Data/S{NNN}/` 디렉토리 생성
- [ ] 시뮬/실험 스크립트 `5_Exp/51_Sim/Scripts/S{NNN}_*.m` 작성
- [ ] 실행 후 결과 .mat + .png 저장
- [ ] Log에 ticket (t{NN}) 추가: 파라미터, 결과, 이미지 경로
- [ ] 작업 완료 시: 얻은 지식을 `7_Log/2_Wiki/`에 요약하고, `S{NNN}_log.md`는 `7_Log/9_Archive/`로 이동
- [ ] 스크립트 완료 시: 1회용 스크립트는 `Scripts/9_Archive/`로 이동, 범용은 Root 유지
- [ ] `7_Log/2_Wiki/Session_Registry.tsv`에 해당 세션 항목 추가 업데이트
