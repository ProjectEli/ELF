...
| **금지 사항** | 아이디어, 논문 기획, 방향성 논의는 `2_Log/`에 기록하지 말고 `1_Concept/13_Planning/`의 기획 문서로 분리합니다. |
...
- [ ] 작업 완료 시: 얻은 지식을 `2_Log/2_Wiki/`에 요약하고, `S{NNN}_log.md`는 `2_Log/9_Archive/`로 이동
- [ ] 스크립트 완료 시: 1회용 스크립트는 `Scripts/9_Archive/`로 이동, 범용은 Root 유지
- [ ] `2_Log/2_Wiki/Session_Registry.tsv`에 해당 세션 항목 추가 업데이트
